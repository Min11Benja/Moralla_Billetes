//Leer pulsos del Monedero + NV10USB y sumarlos/restarlos como crwditos, y mostrar en lcd por min11benja para Morralla
/*-----( Librerias )-----*/
...
//***********************************************************************************************
const int coin_pin = 21; //Asigno el valor de 21 para la variable donde conectare el pin que va a recibir los pulsos del monedero
const int bill_pin = 20; //pin para el NV10USB 20
const int sensor_hopper_pin = 19; // pin para los 3 sensores, no uso uno para cada uno por que solo uno estara prendido y mandando seales a la vez asi que no veo el por que no se pueda usar un solo pin para los 3 dif sensores
const int relevador_A1_pin=4;//A1:relevador que activa el hopper de $1
const int relevador_C5_pin=7;//C5:relevador que activa el hopper de $5
const int relevador_D10_pin=8;//D10:relevador que activa el hopper de $10
...
//pinMode: Configura el pin especificado para comportarse ya sea como una entrada o una salida: pines digitales se pueden utilizar como entrada, INPUT_PULLUP o SALIDA. Cambiar el pin con la funcion pinMode () cambia el comportamiento eléctrico de la clavija.////*input: de afuera hacia el arduino - output*del arduino ahcia afuera - a pull-up* es la resistecia interna del arduino, que asegura que el pin este ya sea en un estado ALTO o BAJO, mientras emplea un nivel bajo de corriente.
  ...
  pinMode(sensor_hopper_pin, INPUT_PULLUP); //set pin 3 as optical input

  pinMode(relevador_A1_pin, OUTPUT);pinMode(relevador_C5_pin, OUTPUT);pinMode(relevador_D10_pin, OUTPUT);//pongo los pins de los relevadores como outputs
  digitalWrite(relevador_A1_pin, HIGH);digitalWrite(relevador_C5_pin, HIGH);digitalWrite(relevador_D10_pin, HIGH);//los pongo por default en apagado desde un inicio
...
//// * OJO: Dentro de la función de "attachInterrupt()", la funcion "delay()" no funcionará y el valor devuelto por "millis()" no se incrementará. Los datos recibidos mientras dentro de la funcion podran ser perdidos. Usted debe declarar como volátiles cualquier variable que modifique dentro de la función adjunta. Vea la sección de ISR a continuación para obtener más información.
  ...
  attachInterrupt(digitalPinToInterrupt(sensor_hopper_pin), restar_credit, FALLING);//agrego interuptor al pin del NV10USB
...
/*--(INTERRUPCION )---*/
void sumar_credit() //funcion invocada para incrementar el contador credit
{
  credit++;
}/*--(end interupcion )---*/

/*--(INTERRUPCION )---*/
void restar_credit() //funcion invocada para disminuir el contador credit
{
  credit--;
}/*--(end interupcion )---*/
